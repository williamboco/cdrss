	...
	while ($row = $result->fetch_assoc()) {
		if ($row['isActive'] == $isActive) {
    		...
    		//$email = openssl_decrypt(base64_decode($row['email']), $method, $key, OPENSSL_RAW_DATA, $iv);
				...
				$msg = '';
				if ($rownum1 > 0) {
					$msg = "These are the medicines/supplies that are in <strong>Threshold Level</strong>:";
				...
				if ($rownum2 > 0) {
					$msg .= "These are the medicines/supplies that are in <strong>Critical Level</strong>:";
					$msg .= "<table border=1><tr><th>Medicines/Supplies</th><th>Available Quantity</th></tr>";
